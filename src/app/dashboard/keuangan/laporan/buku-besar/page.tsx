...
export default function BukuBesarPage() {
  ...
  useEffect(() => { if (periodeId && akunId) load() }, [periodeId, akunId])
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <SelectField label="Akun" value={akunId} onChange={(e) => setAkunId(e.target.value)} options={[{ value: "", label: "Pilih Akun" }, ...akuns.map((a) => ({ value: String(a.id), label: `${a.kode} - ${a.nama}` }))]} className="w-64" />
          <Button variant="ghost" size="sm" onClick={load}><RefreshCw className={`h-3.5 w-3.5 ${loading ? "animate-spin" : ""}`} /></Button>
          <Button variant="ghost" size="sm" onClick={() => downloadCsv("buku-besar.csv", exportRows)}><Download className="h-3.5 w-3.5 mr-1" />CSV</Button>
              ...
              <tbody>
                {data.rows.map((r, i) => (
                  <tr key={`${r.nomor_jurnal}-${i}`} className="border-t" style={{ borderColor: "var(--border)" }}>
                    ...
                    <td className="p-2 font-mono text-xs">{r.nomor_jurnal}</td>
