...
interface OvertimeRequest {
  ...
  jam_mulai_rencana: string; jam_selesai_rencana: string
  durasi_rencana_menit: number; alasan_lembur: string; status: string
...
export default function ApprovalLemburPage() {
  ...
  const { data, loading, refetch } = useApi<OvertimeRequest[]>("/api/sdm/overtime-requests/approval-pending")
  const { data: historyData, loading: historyLoading, refetch: refetchHistory } = useApi<OvertimeRequest[]>("/api/sdm/overtime-requests/approval-history")
  const list = data ?? []
  ...
  const columns: Column<OvertimeRequest>[] = [
    {
      ...
      cell: (r) => r.status === STATUS_LEMBUR.SUBMITTED
        ? <Badge variant="warning" className="text-[10px]">Level 1 — Atasan</Badge>
    ...
    { key: "tanggal_lembur", header: "Tanggal & Jam",
      cell: (r) => <div><p className="font-mono text-sm">{formatDate(r.tanggal_lembur)}</p><p className="text-xs font-mono" style={{ color: "var(--text-subtle)" }}>{r.jam_mulai_rencana.slice(0,5)} – {r.jam_selesai_rencana.slice(0,5)}</p></div> },
    ...
    { key: "overtime_settings", header: "Tipe",
      cell: (r) => r.overtime_settings ? <Badge variant="secondary" className="text-[10px]">{r.overtime_settings.tipe_hari.replace("_", " ")}</Badge> : <span style={{ color: "var(--text-subtle)" }}>—</span> },
  ]
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold" style={{ color: "var(--text-900)" }}>Approval Lembur</h1>
          <p className="text-xs mt-0.5" style={{ color: "var(--text-subtle)" }}>Daftar lembur yang menunggu persetujuan — {list.length} pengajuan</p>
        </div>
      ...
      {list.length === 0 && !loading ? (
        ...
        <DataTable data={list as unknown as Record<string, unknown>[]} columns={columns as unknown as Column<Record<string, unknown>>[]}
          ...
          actions={(row: Record<string, unknown>) => {
            ...
            const isL2 = r.status === STATUS_LEMBUR.APPROVED_SUPERVISOR
            return (
              <div className="flex items-center justify-center gap-1">
                ...
                {canReassignApprover && r.status === STATUS_LEMBUR.SUBMITTED && (
      ...
      <div className="rounded-xl overflow-hidden" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
        <div className="px-4 py-3 flex items-center justify-between" style={{ borderBottom: "1px solid var(--border)" }}>
          <div><p className="text-sm font-semibold" style={{ color: "var(--text-900)" }}>Riwayat Approval Lembur</p><p className="text-xs" style={{ color: "var(--text-subtle)" }}>Pengajuan yang sudah disetujui level 1 atau level 2</p></div>
        ...
        {historyLoading ? <div className="h-40 animate-pulse" style={{ background: "var(--surface-muted)" }} /> : historyList.length === 0 ? <div className="py-10 text-center text-sm" style={{ color: "var(--text-subtle)" }}>Belum ada riwayat approval.</div> : (
          <div className="overflow-x-auto"><table className="w-full text-sm" style={{ borderCollapse: "collapse" }}><thead style={{ background: "var(--surface-muted)" }}><tr>{["Level", "Karyawan", "Tanggal", "Jam", "Durasi", "Diproses Oleh", "Waktu", "Catatan"].map(h => <th key={h} className="px-3 py-2 text-left text-[11px] uppercase tracking-wide" style={{ color: "var(--text-muted)", borderBottom: "1px solid var(--border)" }}>{h}</th>)}</tr></thead><tbody>
            {historyList.map(r => <tr key={`${r.id}-${r.approval_history?.approval_level}-${r.approval_history?.approved_at}`}>
              ...
              <td className="px-3 py-2 font-mono text-xs whitespace-nowrap" style={{ borderBottom: "1px solid var(--border)" }}>{formatDate(r.tanggal_lembur)}</td>
              <td className="px-3 py-2 font-mono text-xs whitespace-nowrap" style={{ borderBottom: "1px solid var(--border)" }}>{r.jam_mulai_rencana.slice(0,5)} - {r.jam_selesai_rencana.slice(0,5)}</td>
              <td className="px-3 py-2 font-mono whitespace-nowrap" style={{ borderBottom: "1px solid var(--border)" }}>{formatMenit(r.durasi_rencana_menit)}</td>
      ...
      {/* Detail */}
      <Modal open={detailOpen} onClose={() => setDetailOpen(false)} title="Detail Lembur" size="md"
        footer={
          <div className="flex gap-2">
            {canReassignApprover && selected?.status === STATUS_LEMBUR.SUBMITTED && (
            ...
            <Button size="sm" style={{ background: "var(--success)", color: "#fff" }} onClick={() => { setActionType("approve"); setActionNote("") }}>
              <Check className="h-3.5 w-3.5 mr-1.5" />{selected?.status === STATUS_LEMBUR.APPROVED_SUPERVISOR ? "Setujui (HRD)" : "Setujui (Atasan)"}
            </Button>
        ...
        {selected && (
          <div className="space-y-3">
            {[
              ...
              { label: "Jam Rencana", value: `${selected.jam_mulai_rencana.slice(0,5)} – ${selected.jam_selesai_rencana.slice(0,5)}` },
              { label: "Durasi",      value: formatMenit(selected.durasi_rencana_menit) },
              { label: "Alasan",      value: selected.alasan_lembur },
            ].map(item => (
      ...
      <ReassignApproverModal
        ...
        endpointBase="/api/sdm/overtime-requests"
        title="Ganti Approver Lembur"
        description={selected ? `${selected.karyawans?.nama_karyawan ?? "Karyawan"} — ${formatDate(selected.tanggal_lembur)}` : undefined}
      ...
      <Modal open={!!actionType} onClose={() => setActionType(null)}
        title={(() => {
          ...
          const isL2 = selected.status === STATUS_LEMBUR.APPROVED_SUPERVISOR
          return actionType === "approve" ? (isL2 ? "Setujui Lembur — HRD Final" : "Setujui Lembur — Atasan") : (isL2 ? "Tolak Lembur — HRD" : "Tolak Lembur — Atasan")
        })()} size="sm"
        footer={<>
          ...
          <Button onClick={handleAction} disabled={actionSaving}
            ...
            {actionSaving ? "Memproses..." : actionType === "approve" ? (selected?.status === STATUS_LEMBUR.APPROVED_SUPERVISOR ? "Setujui (HRD)" : "Setujui (Atasan)") : "Tolak"}
          </Button>
        </>}
      >
        <div className="space-y-3">
          {selected && <p className="text-sm" style={{ color: "var(--text-subtle)" }}><strong>{selected.karyawans?.nama_karyawan}</strong> — {formatDate(selected.tanggal_lembur)} ({formatMenit(selected.durasi_rencana_menit)})</p>}
