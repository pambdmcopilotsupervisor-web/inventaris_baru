...
import { Plus, Eye, Pencil, Trash2, RefreshCw, Search, ArrowRight, Info, FileSpreadsheet, FileText } from "lucide-react"
...
export default function MutasiKaryawanPage() {
  ...
  const handleExport = async (format: "excel" | "pdf") => {
    setExporting(format)
    try {
      const response = await fetch(`/api/mutasi-karyawan/export/${format}`)
      if (!response.ok) throw new Error(`Gagal mengunduh ${format === "pdf" ? "PDF" : "Excel"} mutasi karyawan`)

      const blob = await response.blob()
      const url = URL.createObjectURL(blob)
      const link = document.createElement("a")
      link.href = url
      link.download = `Mutasi_Karyawan_${new Date().toISOString().slice(0, 10)}.${format === "pdf" ? "pdf" : "xlsx"}`
      ...
      window.alert(error instanceof Error ? error.message : "Gagal mengunduh file mutasi karyawan")
    } finally {
      setExporting(null)
    }
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <Modal
        ...
        size="sm"
        title="Export/Cetak Mutasi Karyawan"
        footer={<>
