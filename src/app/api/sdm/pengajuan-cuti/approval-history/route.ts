...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (isAdmin) {
      levelConditions.push({ approval_level: { in: [1, 2] } })
    } else {
      if (isHrd) levelConditions.push({ approval_level: 2 })
      if (isAtasan && karyawanId) {
        levelConditions.push({ approval_level: 1, OR: [{ approver_id: BigInt(karyawanId) }, { approver_user_id: userId }] })
      }
    }

    const approvals = await prisma.leave_request_approvals.findMany({
    ...
    const data = approvals.map(a => ({
      ...
      approval_history: {
        ...
        diproses_oleh_nama: a.approver_user_id ? userMap.get(a.approver_user_id.toString()) ?? null : null,
      },
    }))

    return NextResponse.json(serialize(data))
  } catch (err) {
    console.error("[approval-history cuti]", err)
