...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (pengajuan.status === STATUS_SAKIT.SUBMITTED) {
      if (pengajuan.status !== STATUS_SAKIT.SUBMITTED) return NextResponse.json({ error: "Status tidak valid" }, { status: 422 })
      newStatus = STATUS_SAKIT.REJECTED_SUPERVISOR; approvalLevel = 1
    } else if (pengajuan.status === STATUS_SAKIT.APPROVED_SUPERVISOR) {
      if (pengajuan.status !== STATUS_SAKIT.APPROVED_SUPERVISOR) return NextResponse.json({ error: "Status tidak valid" }, { status: 422 })
      newStatus = STATUS_SAKIT.REJECTED_HRD; approvalLevel = 2
    } else {
      return NextResponse.json({ error: "Status tidak valid" }, { status: 422 })
    }

    const ap = pengajuan.sakit_approvals.find(a => a.approval_level === approvalLevel && a.status === "pending")
    ...
    await prisma.$transaction(async (tx) => {
      await tx.sakit_approvals.update({ where: { id: ap.id }, data: { status: "rejected", approver_user_id: userId, note: body.note.trim(), approved_at: now, updated_at: now } })
      await tx.pengajuan_sakits.update({ where: { id: BigInt(id) }, data: { status: newStatus, updated_at: now } })
      await writeAuditLog({ user: auth.user, action: "UPDATE", modelType: "pengajuan_sakits", modelId: BigInt(id), dataBaru: { status: newStatus, note: body.note }, ip: getClientIp(req), tx })
    })
    return NextResponse.json({ success: true, message: "Sakit ditolak" })
  } catch (err) {
    console.error("[reject sakit]", err)
